...
`cdb_capture_dvbt_registers.txt`はレジスタ書き込みヘルパー（`mnservice+0x88500`/`0x883b0`）に
`r r8; r r9; g`（アドレス・データをダンプして即座に継続、止まらない）を仕掛ける版——
「どのMode/フィールドがどのレジスタに何を書き込むか」を丸ごと捕捉したい時に使う
（続報15でDVB_Tモードのレジスタ書き込みを捕捉した際に使用）。

**注意（重要な教訓）**: cdbでのブレークポイントは1ヒットごとに実行を止めてコマンドを実行する
オーバーヘッドがあるため、**タイミングに敏感な実機とのやり取りを壊す場合がある**。実際に
DVB_Tモードの`ChannelStart`は素の速度では毎回成功するが、この`cdb_capture_dvbt_registers.txt`
下で実行すると`_MODCMD_START Fail`という実機側のネイティブエラーが新たに発生した——
デバッガのオーバーヘッドがハードウェアハンドシェイクの時間的制約を壊した可能性が高い。
**cdbキャプチャ下で失敗が観測されても、素の速度でも失敗するとは限らない**ことに注意
（逆に、cdb下で成功した場合は素の速度でも成功する可能性が高いと考えてよい）。
