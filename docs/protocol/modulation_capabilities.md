...
### 続報37 (2026-07-30): 直接TS実験をロールバック、公式TSは完全な再多重化と判明

続報35・36で追加したCBRキュー、NULL補充、`0x0629` ACK、較正読み出しは受信品質を解決せず、
先行投入はロックを悪化させた。直接USBのTS送信コードを`6f834dd`時点の単純な実装へ戻した。

USBPcapの公式bulk 325 slice（41,600 packet）をTSへ抽出してTSDuckで解析したところ、
入力録画TS（4サービス、40 PID、13,219,474 bit/s）とは別物だった。mnservice出力は
1サービス、12 PID、PCR基準7,159,151 bit/sで、TSID/ONID/NID=`0x7E81`、
Service ID=`0x5C08`、PCR=`0x0100`、PMT=`0x0101`、video=`0x0110`、audio=`0x0120`。
PAT/CAT/NIT/SDT/EIT/TOT/BITも新規生成されている。

一方、従来のGUI相当TSDuck列をオフラインで再現すると、最初のサービスをID 1/VAT-01へ
変更するだけで残り3サービス、TSID/ONID、元局NITのエリア・周波数一覧、35 PIDが残った。
`nit --create`は既存NITを置換しない。この不整合はTVTestのチャンネル検出不能を説明できる。
...
### 続報39 (2026-07-30): サービス経由1080iの低ビットレート既定値を修正

mnservice経由をTVTestで受信した映像は約3.08 Mbpsで、細かい模様や動きの多い1080i素材では
大きな量子化ブロックが見えた。RFエラーとは別に、GUIの`BitrateRatio=50`とQPSK既定値が
映像容量を強く制限していた。

GUIのフルセグ既定を64QAM・符号化率3/4・GI 1/8へ揃え、映像レートを85、
VBR最低/最高を60/100、VBR画質目標を75へ変更した。DQPSK/QPSKと1080p/1080iを組み合わせた
場合は、容量不足になりやすいことと64QAM推奨をログへ表示する。これらは画質寄りの既定値で、
実際の受信環境で誤りが増える場合は変調方式または符号化率を耐性側へ戻す。

### 続報40 (2026-08-04): ISDB-T容量見積もりをGUIに追加

mnservice公式出力で実測したQPSK・5/6・GI 1/16の7,159,151 bit/sを基準に、
変調多値数、符号化率、GI効率の比率から13セグTS容量を見積もる
`IsdbTBitrate`を追加した。64QAM・3/4・GI 1/8の見積もりは18,255,835 bit/s。

サービス経由のChannelStart前に容量をログ表示し、直接USBのTSビットレート欄に
「変調から計算」を追加した。古い固定既定値16,851,216 bit/sは、現在の
64QAM・3/4・GI 1/8と合わないため18,255,835 bit/sへ更新した。これは総TS容量であり、
映像ESの実ビットレートはエンコーダ設定とPSI/SI・音声・null packet分を除いた値になる。
