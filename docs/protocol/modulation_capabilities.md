...
受信、最初の128 packetをUSBスライスへ組み立てた段階で、続報26と同じデバイス側未消費状態に
より同期bulk writeが待機した。8秒後にTSDuckを終了、UDP socketを閉じ、`WinUsb_AbortPipe`で
待機を解除してからRF停止コマンドを送信できた。子プロセスの残留はなく、実機は
