...
入力サービス選択UIは次の課題として残る。また、
PSI/SIの構造が正しくなったことと、直接USB bulk経路に残る高TEI問題の解決は別である。

2026-08-04にPCR/CBR調整の第一段階を実装した。送出開始前に`tsanalyze --normalized`で
入力TSの先頭サービスとPCRビットレートを1回で取得し、目標容量との差から
`--add-input-stuffing` の比率を自動生成する。例の13,219,474 bit/s素材を
18,255,835 bit/sへ合わせる場合、固定の`1/20`ではなく約38.1%のnull packetを追加する。
入力PCRレートが目標以上の場合は、時間軸を壊して押し込まず明示的に停止する。

これで「平均レートが変調容量に届かない」問題は解消するが、公式経路のような
複数overlapped USB転送による短期バーストはまだ再現していない。

## 公式bulk転送タイミング

`Measure-XHeadBulkTiming.ps1`で公式キャプチャの325回の24,064-byte bulk OUTを測定すると、
先頭から末尾まで7.853秒、平均間隔24.239 ms、USB payload平均7,966,806 bit/sだった。
間隔は一定ではなく、3 ms未満が42回、10–30 msが230回、30 ms以上が52回（最大188.835 ms）
ある。公式サービスのPCRレート7.159 Mbpsに近い平均を保ちつつ、短い間隔で複数スライスを
投入するバーストがある。

現在の直接USB実装は同期`WinUsb_WritePipe`を1本ずつ完了待ちするため、この投入形状を
再現できない。高TEIの次の有力候補は「平均ビットレート」だけでなく、公式と同様に複数の
overlapped bulk転送を保留してデバイス側リングを枯らさない方式である。実装時は停止時の
`WinUsb_AbortPipe`、各OVERLAPPEDの完了回収、固定バッファの寿命管理を一体で行う必要がある。
