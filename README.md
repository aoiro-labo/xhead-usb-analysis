...
| チャンネル/番組メタデータ（サービス名・NetworkID等）の変更 | **未着手・危険と判明** | [docs/protocol/modulation_capabilities.md](docs/protocol/modulation_capabilities.md)「続報14」— GUIに編集タブを実装したが、`mMTSChannelParam`/`mMTSProgramParam`のどのフィールドを明示的に上書きしても（既存値と同一のno-op書き込みですら）`ChannelStart`が`mnservice.exe`をサービス全体ごとハングさせることが切り分けテストで判明、機能を撤去した。実機ハードウェアは6回のハング全てで健全（`direct_usb`で確認）。CLIの切り分けテスト(`--meta <subset>`)は今後の調査用に残存 |
