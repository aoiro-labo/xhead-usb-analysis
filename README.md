...
| `mnservice.exe`ネイティブ側の生USBプロトコル解析 | 検証中 | [tools/usb_capture](tools/usb_capture) — USBPcapで実データキャプチャに成功。バルク転送(24064バイト=MPEG-TS 188バイト×128)の生TSフレーミングを確認、ベンダーコントロール転送のハンドシェイクパターンも判明。詳細なバイト意味論は未解読 |
