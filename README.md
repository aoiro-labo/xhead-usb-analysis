...
| ISDB-T以外のMode切替（DVB_T等、STUDIOにない機能） | **8モード全て実施完了：7モードがdirect_usbから送出可能・DVB_T2のみ未解決** | [docs/protocol/modulation_capabilities.md](docs/protocol/modulation_capabilities.md)「続報12・13・22・24」— 当初「field not exists」で拒否されていたのは本ツール側の送信フォーマットの不備（旧モードの固有フィールド削除漏れ）と判明・修正。`DVB_T`/`ATSC`/`J83B`は`mnservice.exe`経由の`ChannelStart`まで完走しRF出力を実測。**`DTMB`/`J83C`は`mnservice.exe`経由だとサービス全体をハングさせるが、`direct_usb`単体ならハングせず送出可能と実証（続報22）**。**`J83A`は`mnservice.exe`独自のビットレート検証がシンボルレート的な値を要求するがプロパティ記述子に設定手段が無いため常に拒否される、というソフトウェア側の実装不備と判明（Ghidra静的解析）、`direct_usb`単体では問題なくRF出力に成功（続報24）**。これで7モード（DVB_T/J83A/ATSC/J83B/DTMB/ISDB_T/J83C）が`direct_usb`単体で送出可能に。`DVB_T2`のみ、正真正銘のDVB-T2規格準拠パラメータ検証で拒否されており（同じくGhidra解析で確認）、正しいパラメータの組み合わせが未特定のまま残る（2種の代替組み合わせを試したが失敗、実害なし）。ビットレベルでの規格準拠は未検証。**アンテナ接続での非ISDB-T送出は電波法上の理由から行わないこと** |
