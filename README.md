...
| ISDB-T以外のMode切替（DVB_T等、STUDIOにない機能） | **DVB_Tで実証済み** | [docs/protocol/modulation_capabilities.md](docs/protocol/modulation_capabilities.md)「続報12」— 当初「field not exists」で拒否されていたのは本ツール側の送信フォーマットの不備（旧モードの固有フィールド削除漏れ）と判明・修正。`Mode=DVB_T`への切替は`ChannelStart`まで完走し、ISDB_Tモードと同水準（+37〜39dB）のRF出力を470〜476MHz帯で実測（`tools/custom_sender --dvbt`）。ビットレベルでの規格準拠は未検証、他7規格は未実施。**アンテナ接続での非ISDB-T送出は電波法上の理由から行わないこと** |
