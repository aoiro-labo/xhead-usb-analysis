...
| ISDB-T以外のMode切替（DVB_T等、STUDIOにない機能） | **8モード全て実施完了：成功3（mnservice.exe経由）+2（direct_usb限定）・安全な拒否2** | [docs/protocol/modulation_capabilities.md](docs/protocol/modulation_capabilities.md)「続報12・13・22」— 当初「field not exists」で拒否されていたのは本ツール側の送信フォーマットの不備（旧モードの固有フィールド削除漏れ）と判明・修正。`DVB_T`/`ATSC`/`J83B`は`mnservice.exe`経由の`ChannelStart`まで完走しRF出力を実測（`tools/custom_sender --dvbt/--atsc/--j83b`）。`J83A`/`DVB_T2`は安全な値検証拒否（実機到達せず、未検証のまま）。**`DTMB`/`J83C`は`mnservice.exe`経由の`ChannelStart`がサービス全体ごとハングさせる**（`--dtmb`/`--j83c`は`mnservice.exe`経由では使用非推奨）**が、続報22で`mnservice.exe`を一切経由しない`direct_usb`単体であれば両方ともハングせず正常にRF出力まで到達することを実証**——STUDIO本体では実現不可能なモードを独自ツールでのみ実現した。ビットレベルでの規格準拠は未検証。**アンテナ接続での非ISDB-T送出は電波法上の理由から行わないこと** |
